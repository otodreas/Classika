...
def get_species(names, dataset_type):
    ...
    for name in names:
        ...
        if dataset_type == 'canids':
            ...
            parts = name.split('_')
            sp = '_'.join(parts[:2]) if len(parts) >= 2 else (parts[0] if parts else name.strip())
            species.append(sp)
...
def compute_centroid_features(landmarks):
    ...
    for i in range(n_samples):
        ...
        centroid_dists = np.array([np.linalg.norm(landmarks[i, j] - centroid) 
                                   for j in range(n_landmarks)])
...
def compute_shape_descriptors(landmarks):
    ...
    for i in range(n_samples):
        ...
        feat.append(bbox_dims[0] / (bbox_dims[1] + 1e-10))  # Aspect ratio (always valid)
        if n_dims == 3:
            feat.extend([
...
def create_stacking_ensemble(n_classes):
    ...
    if HAS_CATBOOST:
        base_estimators.append(('cb', cb.CatBoostClassifier(
    ...
    base_estimators.extend([
        ...
        ('et', ExtraTreesClassifier(n_estimators=150, random_state=RANDOM_STATE, n_jobs=-1)),
        ('svm', SVC(kernel='rbf', C=10, probability=True, random_state=RANDOM_STATE)),
...
def create_blending_ensemble(X_train, y_train, X_test, n_classes, blend_holdout=0.3, meta_C=1.0):
    ...
    X_blend_train, X_blend_hold, y_blend_train, y_blend_hold = train_test_split(
        X_train, y_train, test_size=blend_holdout, random_state=RANDOM_STATE, stratify=y_train
    )
    ...
    meta_learner = LogisticRegression(max_iter=1000, C=meta_C, random_state=RANDOM_STATE)
...
def create_weighted_voting_with_params(n_classes, model_scores, top_k=6, weight_power=1.0):
                    ...
                    for train_idx, test_idx in skf.split(X, y):
                        ...
                        try:
                            y_pred = create_blending_ensemble(
                                ...
                                blend_holdout=blend_holdout, meta_C=meta_C
                            )
                            fold_scores.append(accuracy_score(y_test, y_pred))
                        except Exception:
                            fold_scores.append(0)
                    per_dataset_scores.append(float(np.mean(fold_scores)) if fold_scores else 0)
                avg = float(np.mean(per_dataset_scores)) if per_dataset_scores else -1
                if avg > best["cv_accuracy"]:
                    best.update({
...
def build_confusion_matrix_for_model(
    ...
    ax.set_xticklabels(class_names, fontsize=10, fontweight='bold', rotation=45, ha='right')
    ...
    cm_filename = os.path.join(output_dir, f'{dataset_name}_Confusion_Matrix_Best_Overall.png')
    plt.savefig(cm_filename, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"    [OK] Confusion matrix saved: {cm_filename}")
    print(f"    [OK] Overall Accuracy: {acc:.2f}% ({N_SPLITS}-fold CV, {len(y)} specimens)")
    ...
    if os.path.exists(classifier_path):
        try:
            ...
            for _, row in classifier_df.iterrows():
                ...
                key = f"{inst}_{spec}"
                classifier_dict[key] = {
                    'nose': str(row.get('Nose', '')).strip() if pd.notna(row.get('Nose')) else '',
                    'bitework': str(row.get('Bitework', '')).strip() if pd.notna(row.get('Bitework')) else '',
                    'ukc': str(row.get('UKC breeding Standard', '')).strip() if pd.notna(row.get('UKC breeding Standard')) else '',
                    'akc': str(row.get('AKC breeding standard', '')).strip() if pd.notna(row.get('AKC breeding standard')) else ''
                }
    ...
    if os.path.exists(breed_path):
        try:
            ...
            for _, row in breed_df.iterrows():
                ...
                spec = str(row['Specimen']).strip()
                key = f"{inst}_{spec}"
                breed_dict[key] = str(row.get('Breed', '')).strip() if pd.notna(row.get('Breed')) else ''
...
def main():
    ...
    datasets = [
        ...
        ('Papionins', 'Papionins/raw_data/cercocebus,macaca mandrilus, papio and lophocebus adults .txt', 'papionins'),
    ...
    for name, path, dtype in datasets:
        ...
        if not os.path.exists(path_resolved):
            alt_paths = [
                path_resolved,
                path,
                os.path.join(PROJECT_ROOT, os.path.dirname(path).replace('raw_data', ''), os.path.basename(path)),
                os.path.join(PROJECT_ROOT, 'raw_data', os.path.basename(path)),
            ]
            if 'wolf' in path.lower():
                alt_paths.extend([
                    os.path.join(PROJECT_ROOT, 'Wolves', 'raw_data', os.path.basename(path)),
                    os.path.join(PROJECT_ROOT, 'wolf', 'raw_data', os.path.basename(path)),
                ])
            found = False
    ...
    for idx, (name, path, dtype) in enumerate(datasets, 1):
        ...
        if result:
            ...
            summary_info = {
                ...
                'best_accuracy_pct': result.get('best_accuracy_pct', result['best_accuracy'] * 100)
            }
            ...
            try:
                with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
                    pd.DataFrame(results_data).to_excel(writer, sheet_name='All_Results', index=False)
                    pd.DataFrame(summary_data).to_excel(writer, sheet_name='Summary', index=False)
                    pd.DataFrame(best_models_data).to_excel(writer, sheet_name='Best_Models', index=False)
    ...
    for s in all_summaries:
        ...
        print(f"  {s['dataset']:<12} {s['specimens']:<6} {s['landmarks']:<4} {s['features']:<6} {s['classes']:<4} {s['best_model']:<22} {s['best_accuracy']:.4f} {acc_pct:>10.2f}%")
    
    avg_acc = np.mean([s['best_accuracy'] for s in all_summaries])
    avg_acc_pct = avg_acc * 100
    print("  " + "-"*85)
    print(f"  {'AVERAGE':<12} {'':<6} {'':<4} {'':<6} {'':<4} {'':<22} {avg_acc:.4f} {avg_acc_pct:>10.2f}%")
    print("="*85)
    ...
    if len(model_averages) > 0:
        ...
        sorted_models = sorted(
            ...
            key=lambda x: (-x[1]['average'], -min(s['accuracy'] for s in x[1]['scores']))
        )
        ...
        for model_name, data in sorted_models:
            avg = data['average']
            scores_str = ", ".join([f"{s['dataset']}: {s['accuracy']:.3f}" for s in data['scores']])
            print(f"  {model_name:<25} {avg:.4f}          {avg*100:>10.2f}%    {scores_str}")
        ...
        print(f"  *** Average Accuracy: {best_overall_avg:.4f} ({best_overall_avg*100:.2f}%) ***")
        print(f"  {'='*70}")
        
        print(f"\n  Per-Dataset Performance:")
        for s in best_overall_data['scores']:
            print(f"    {s['dataset']}: {s['accuracy']:.4f} ({s['accuracy']*100:.2f}%)")
    else:
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        if len(model_averages) > 0:
            sorted_models = sorted(model_averages.items(), key=lambda x: -x[1]['average'])
            ...
            best_overall_df.to_excel(writer, sheet_name='Best_Overall_Model', index=False)
        ...
        if tuned_voting_params:
            ...
            for dataset_name, params in tuned_voting_params.items():
                tuned_rows.append({
                    ...
                    'Weights': ", ".join([f"{w:.6f}" for w in params.get('weights', [])]),
